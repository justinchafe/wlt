...
public class WltPanel extends JPanel implements ComponentListener, MouseListener, MouseMotionListener, ActionListener {
	...
	boolean drawLine, hasPainted, drawBottle, hasClicked, hasDragged;
	Slides mySlides;
	JLabel testLabel;
...
//case n+2: final page
	...
	public WltPanel(Participant p, Wlt parentFrame) {
            wltParent = parentFrame;
            wltParent.hideResetButton();    
            nm = new NetworkManager(Wlt.SERVER, Wlt.PORT);
            sThread = new Thread() {
                public void run() {
                    if (nm.createSocket()) {
                        JOptionPane.showMessageDialog(wltParent, "Connected to Eye Tracker!");
                        sendValue(150);
                    }else {
                        JOptionPane.showMessageDialog(wltParent, "Error could not connect to Eye Tracker!");
		...
		ENDSCREEN = -1;	
		hasPainted = hasClicked = drawLine = hasDragged = false;
		...
		mySlides = new Slides("XmlIntroSideData.xml");
	...
	private void addMotionRadioButtons() {
		...
		movingButton = new JRadioButton("Moving?");
   		movingButton.setMnemonic(KeyEvent.VK_B);
    		movingButton.setActionCommand("F");
	...
	public void actionPerformed(ActionEvent event) {
		if (event.getActionCommand().equals(Wlt.BUTTON_MSG2)) {
			l.resetUserLine();
			repaint();	
		...
		if (screenNum ==  0) {
			...
			System.out.println(l.getUserLineLength());
			if (!l.isUserLineDrawn()) {
				...
				mySlides = new Slides("XmlSideData.xml");
				mySlides.loadFromFile();
				...
				hasPainted = drawLine = hasClicked = hasDragged = false;
				setRatios();
				repaint();
			}
        ...
        //THIS MAY NOT REALLY BE NEEED FOR THIS APP!!!!
private void sendValue(int tempint) {
          
            byte[] tempB = {(byte) tempint,0};
           
           // int tempint = Integer.parseInt(dm.value.getText());
           //if (tempint > 255 || tempint < 0) {
             //  JOptionPane.showMessageDialog(wltParent, "incompatible value");
               // tempB[0] = -1;
             /*   
            if (tempint < 255 && tempint >= 0) {
               tempB[0] = (byte) (tempint - 128);
               System.out.println("VALUE TIME:" + tempB[0]);
            }else {
                JOptionPane.showMessageDialog(wltParent, "incompatible value, setting zero");
                tempB[0] = 0;
            }   
            * */
            if (nm.sendValue(tempB) ) {
                ;
            }else
                JOptionPane.showMessageDialog(wltParent, "Error! Could not send XDAT Value!");
